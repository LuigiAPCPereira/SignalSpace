...
| M6 — programação vertical local | Composição opt-in pública com concessão local e escopos independentes para READ, edição, onze tools de filesystem tipado e revisão Git somente leitura, com negação após revogação. `test.run` permanece separado. | WORKSPACE_SECURITY; `PROGRAMMING_TOOLS.md`; promotion gate | SS-MVP-002…006 | base e slice estrutural publicados; `apply_patch` implementado/validado localmente, push pendente; aceite operacional externo pendente | `3628d2f` confirma o remoto live para a base/estrutural; `internal/workspace/patch.go` e `internal/mcp/workspace_patch.go` cobrem o patch fechado, preflight e compensação. Suíte completa, race afetado, vet, build, gofmt e diff-check passaram. Não é HTTPS/ChatGPT Web, workspace real ou sandbox de processo; shell, mutação Git e worktree funcional permanecem fora. |
