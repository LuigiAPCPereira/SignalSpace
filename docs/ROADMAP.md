...
**Estado reconciliado nesta retomada (23/09/2026):** a composição opt-in READ + WRITE + Git observacional de `SS-MVP-002` está publicada em `6b8adc5b44e4856a743f0df336241aff5b5da79e`, e o slice estrutural `copy_path`/`move_path`/`delete_file`/`delete_directory` está implementado e validado localmente em commits ainda não publicados. Isso não é aceite operacional externo/HTTPS/ChatGPT Web e não promove `test.run`, shell, mutação Git, `apply_patch` ou worktree.
...
| M6 — programação vertical local | Composição opt-in pública com concessão local e escopos independentes para READ, edição, dez tools de filesystem tipado e revisão Git somente leitura, com negação após revogação. `test.run` permanece separado. | WORKSPACE_SECURITY; `PROGRAMMING_TOOLS.md`; promotion gate | SS-MVP-002…006 | base publicada; slice estrutural implementado/validado localmente em `347f84b`; aceite operacional externo pendente | Promoção anterior publicada em `e97aaf7`/`d1bff2c`/`6b8adc5`; dez tools em `internal/workspace`/`internal/mcp`; suíte Go serial, race afetado, vet, build, gofmt e diff-check passaram. Não é HTTPS/ChatGPT Web, workspace real ou sandbox de processo; shell, mutação Git e worktree funcional permanecem fora. |
