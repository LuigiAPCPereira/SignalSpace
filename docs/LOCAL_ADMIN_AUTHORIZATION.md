...
Essa decisão não altera as rotas v1, não cria painel funcional adicional, não muda o mecanismo de terminal vigente e não concede escrita, Git ou shell por si só. O ciclo OAuth v2 foi implementado separadamente no auth harness opt-in, com token family, rotação, revogação e vínculo cliente/resource. O núcleo interno de capabilities/policy agora adapta os scopes nas bordas, mas não cria grant público nem migra as tools públicas; `REQUIRE_APPROVAL` continua não exposto. Em particular, a concessão de workspace continua distinta de OAuth; uma futura aprovação `REQUIRE_APPROVAL` deve produzir zero efeito e exigir repetição da tool com revalidação. Policy Engine integrado, approvals persistentes, permits e migração pública continuam posteriores e requerem tarefas próprias.

## Estado implementado — `SS-MVP-002-LOCAL-APPROVAL-POLICIES-UI-V2-001`

O painel local agora registra approvals de capability em rota própria, com decisões explícitas `ALLOW_ONCE`, `ALLOW_SESSION`, `ALLOW_WORKSPACE` e `DENY`. A fila não executa a operação original. A policy store é privada, versionada, limitada, atomicamente atualizada e fail-closed; não contém bearer, refresh token, fingerprint, root ou conteúdo. A política persistente é por owner/client/managed workspace/capability e sua revogação é independente de qualquer escrita. A API exige sessão administrativa, CSRF e transporte same-origin; o runtime MCP público permanece sem a ponte.

## Integração com a ponte Programming v2 — 26/09/2026

Na composição `connect quick programming`, `/api/admin/v1/capability-approvals` é a superfície owner-side que decide requests criados pelo `ProgrammingAuthorizer`. O MCP não chama a API administrativa nem recebe um permit: ambos usam a mesma instância de `approval.Manager`. O fluxo observado localmente por HTTP foi MCP `tools/call` → `LOCAL_APPROVAL_REQUIRED` sem efeito → listagem autenticada → decisão `ALLOW_ONCE` com CSRF → retry idêntico executado uma vez → novo request no terceiro retry.

O contrato permanece local: o painel/servidor administrativo continua em `127.0.0.1:7677`, nunca é destino do Quick Tunnel, e a composição sem painel retorna `LOCAL_APPROVAL_UNAVAILABLE`. Approval não amplia grant, não concede scope OAuth e não autoriza shell, testes, Git remoto ou lifecycle de worktree. Estado: **IMPLEMENTADA / VALIDADA LOCALMENTE / PUBLICADA REMOTAMENTE** em `e617ac15e77d137f07be6cdb7deb69f24a77d101`; Web, túnel, workspace real e CI não foram validados.
...
## Standard Profile owner-side — `SS-MVP-002-PROGRAMMING-STANDARD-PROFILE-V2-001`

O console local oferece uma forma canônica sem lista de scopes fornecida pelo cliente: checkout usa `workspace request-programming <client-id> <absolute-path>` e managed usa `workspace request-worktree <client-id> <source-root> [base-ref]`. A aprovação cria o grant tipado e só então aplica o profile correspondente; falha do profile revoga o grant e, no managed, desativa o worktree criado.
