...
`internal/mcp/oauth_programming_integration_test.go` (commit `2065480`) fecha o fluxo real conjunto: DCR, PKCE, decisão terminal, `/authorize`, `/authorize/complete`, `/token`, claims verificados por `NewStaticJWTVerifier`, tokens separados de escrita/Git/teste, `replace_text` → `run_workspace_tests` → `review_git_changes`, código de autorização de uso único, revogação com JWTs ainda válidos, contadores sem nova operação após revoke e handler público sem as três ferramentas. A mesma composição comprova que authorize sem concessão não cria pedido pendente nem grant. `internal/mcp/oauth_write_lifecycle_test.go` comprova a perda da concessão após fechamento/recriação local mesmo com JWT persistido e a exigência de nova sessão explícita. `internal/auth/write_scope_test.go` e `internal/auth/programming_scope_test.go` cobrem opt-in explícito, texto de consentimento, combinações canônicas, metadata padrão sem os novos escopos e revogação antes de conclusão/troca.
