...
`CapabilityApproval` é a mesma lógica exercitada pelo console e pelo harness OAuth/MCP; o teste vertical público usa a confirmação terminal-local antes de emitir tokens READ/WRITE/GIT. A aprovação terminal-local continua distinta do consentimento OAuth: depois da confirmação, cada token e cada ferramenta ainda exigem seus escopos e a concessão corrente. A composição não adiciona rotas administrativas, executor, shell ou mutação Git aos entrypoints; o revisor Git mantém baseline por sessão e somente status/diff.
