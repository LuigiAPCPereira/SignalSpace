...
**Estado vigente (24/09/2026):** o promotion gate `SS-MVP-002-PROMOTION-GATE-001` foi **APROVADO PELO PROPRIETÁRIO**. O remoto live está em `6fcf46da1f9e1187a8067d8a71a745d31babc618`; a promoção local de `git_status` e `git.index` foi implementada e validada no commit local `e164415`, ainda não publicado. `test.run`, shell, comandos arbitrários, commit/branch/Git remoto e lifecycle MCP de worktree continuam fora. Isso não é aceite de túnel, HTTPS, navegador, grant real ao ChatGPT Web, workspace real, CI, merge ou deploy.
