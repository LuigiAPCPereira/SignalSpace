...
**Estado:** **IMPLEMENTADO / VALIDADO LOCALMENTE / PUBLICADO REMOTAMENTE** em `3b9fefe398b67cf0752f0a8034d22a829b808c38`. `SS-MVP-002` permanece **PARCIAL/em andamento**; a superfície pública MCP/OAuth continua granular e CI permanece **DESCONHECIDA**.

**Ref e publicação:** branch `codex/mvp-vertical-programming`; o ciclo anterior estava publicado em `6c7999b11e6761101c1ecf10e47816c82a0c9587` e o commit focado desta correção avançou por fast-forward para `3b9fefe398b67cf0752f0a8034d22a829b808c38`, confirmado por `git ls-remote`.

**Implementação:** `internal/capability` define o catálogo fechado de capabilities internas, adapta explicitamente os seis scopes OAuth legados e não atribui scopes a capabilities futuras. `internal/workspace/grants.go` oferece `GrantWithCapabilities`, `GrantManagedWithCapabilities` e `AllowsClientCapability`, armazena capabilities tipadas e checa capabilities nas operações; wrappers legados preservam compatibilidade, com `workspace.write` expandindo temporariamente para `workspace.write + workspace.delete`, enquanto grant tipado somente `workspace.write` não autoriza delete. `GrantSnapshot.Capabilities` é explícito e `Scopes` é derivado. `internal/policy` fornece engine em memória com contexto completo, grant ativo/capabilities concedidas, regras `DENY`/`ASK`/`ALLOW_SESSION`/`ALLOW_WORKSPACE`, precedência por especificidade, negação em empate, expiração e retorno `ALLOW`/`DENY`/`REQUIRE_APPROVAL`.

**Validação:** suíte Go completa serial, race proporcional de capability/policy/workspace/auth/MCP/cmd, `go vet ./...`, `go build ./...`, `gofmt` e `git diff --check` passaram. Os testes cobrem mapeamento sem aceitar scope desconhecido, armazenamento sem raw scope, snapshot legado + capabilities, wrappers tipados, delete independente/compatível, decisões fail-closed, `ASK`, allow workspace/session, capability ausente como `REQUIRE_APPROVAL`, grant revogado como `DENY`, precedência, deny em empate, expiração e engine de valor zero.
