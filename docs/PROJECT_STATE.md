...
**Validação:** testes focais cobrem mapeamento sem aceitar scope desconhecido, armazenamento sem raw scope, snapshot legado + capabilities, wrappers tipados, delete independente/compatível, decisões fail-closed, `ASK`, allow workspace/session, capability ausente como `REQUIRE_APPROVAL`, grant revogado como `DENY`, precedência, deny em empate, expiração e engine de valor zero. Os gates completos devem ser repetidos após esta correção antes da publicação.
