...
type WorkspaceGitStatusReader interface {
	GitStatus(context.Context, string, string, string) (workspace.GitIndexStatus, error)
}
...
func gitStatusToolDefinition() map[string]any {
	return map[string]any{
		"name":        gitStatusToolName,
		"description": "Return structured local Git status and an optimistic index fingerprint. Read-only; never stages, commits or pushes.",
		"inputSchema": map[string]any{
			"type":                 "object",
			"properties":           map[string]any{"session_id": map[string]any{"type": "string"}},
			"required":             []string{"session_id"},
...
func (a *gitToolAccess) call(w http.ResponseWriter, ctx context.Context, id any, raw json.RawMessage) {
	...
	payload, err := json.Marshal(struct {
		...
		StagedChange bool              `json:"staged_diff_changed"`
	}{
		Before:       gitReviewSnapshot{Status: review.Before.Status, Diff: review.Before.Diff, StagedDiff: review.Before.StagedDiff, OutputTruncated: review.Before.OutputTruncated},
		After:        gitReviewSnapshot{Status: review.After.Status, Diff: review.After.Diff, StagedDiff: review.After.StagedDiff, OutputTruncated: review.After.OutputTruncated},
		Complete:     review.Complete,
		StatusChange: review.StatusChange,
		DiffChange:   review.DiffChange,
		StagedChange: review.StagedDiffChange,
	})
	if err != nil {
...
func (a *gitToolAccess) status(w http.ResponseWriter, ctx context.Context, id any, raw json.RawMessage) {
	identity, err := a.verify(ctx)
	if err != nil {
		challenge := a.challenge + `, error="invalid_token", error_description="Invalid access token"`
		if errors.Is(err, ErrInsufficientScope) {
		...
		reply(w, http.StatusOK, response{JSONRPC: "2.0", ID: id, Result: map[string]any{
			"content": []any{map[string]any{"type": "text", "text": "Git status authorization required."}},
			"_meta":   map[string]any{"mcp/www_authenticate": []string{challenge}},
			"isError": true,
		}})
		return
	...
	status, err := a.statusReader.GitStatus(ctx, identity.OwnerSubject, identity.ClientID, sessionID)
	if err != nil {
		reply(w, http.StatusOK, response{JSONRPC: "2.0", ID: id, Result: map[string]any{
			"content": []any{map[string]any{"type": "text", "text": "Git status unavailable or not authorized."}},
			"isError": true,
		}})
		return
	}
	...
	if err != nil {
		fail(w, http.StatusOK, id, -32603, "Git status unavailable")
		return
	}
	reply(w, http.StatusOK, response{JSONRPC: "2.0", ID: id, Result: map[string]any{
		"content": []any{map[string]any{"type": "text", "text": string(payload)}},
